...
//
// Project submission by: Sergio Herrera
...
bool place_in_hash_tables (string s) {
  ...
  while((!placed ) && (counter < 2*tablesize)) {
    if (t[pos][index] == "") {
      ...
      temp = t[pos][index]; //store string in t[pos][index] into temp
      t[pos][index] = temp_s; //update t[pos][index] with temp_s
      ...
      index = index ? 0 : 1; //depending on value of index, apply hash function f1 (index = 0) or f2 (index = 1)

      // WRITE THE CODE TO CALCULATE IN pos THE HASH VALUE FOR temp_s
      pos = f(temp_s, index);

      counter ++; //iterate to next string
    }
  }
  return placed;
};
...
size_t f(string s, size_t index) {
  ...
  if (index == 0) {
    ...
    val = s[len-1]; //starting index is last index in table 2
    val = val % tablesize; //value for first calculation of string in last index
    ...
    //string length longer than 1 so iterate through
    for (i = len-2; i >= 0; i--)
    {
        temp = s[i]; //index len-i-1 from formula of function f2
        po *= prime; //multiply position by prime number 41
        po = po % tablesize; //position of string in table
        ...
        val += temp * po; //update f2 value with position value
        val = val % tablesize; //update value
    ...
    return val; //return value for function f2
  }
}
